...
document.getElementById('generate-sketch-form').addEventListener('submit', async function (e) {
  ...
  const sketchImage = document.getElementById('generated-sketch');
  
  // Показываем загрузку
  placeholder.innerHTML = '<i class="fas fa-spinner fa-spin"></i><p>Генерация эскиза...</p>';
  
  try {
    // Здесь должен быть реальный запрос к API
    // Это примерная реализация
    const response = await fetch('https://api.kandinsky.ru/generate', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'EF17F2E249F2A0C1548DFA9F4A2EEEAA'
      },
      body: JSON.stringify({
        prompt: description,
        width: 512,
    ...
    const imageUrl = data.image_url; // Предполагаем, что API возвращает URL изображения
    
    // Показываем результат
    sketchImage.src = imageUrl;
    ...
    // Обработчик для перегенерации
    const regenerateBtn = content.querySelector('.regenerate-btn');
    regenerateBtn.onclick = () => {
      content.classList.add('hidden');
      placeholder.innerHTML = '<i class="fas fa-spinner fa-spin"></i><p>Генерация нового варианта...</p>';
      placeholder.classList.remove('hidden');
      // Здесь можно автоматически отправить форму снова
      this.dispatchEvent(new Event('submit'));
    };
    
  } catch (error) {
    placeholder.innerHTML = `<i class="fas fa-exclamation-triangle"></i><p style="color: #ff6f61;">Ошибка: ${error.message}</p>`;
  }
});